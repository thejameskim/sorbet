...
unique_ptr<CFG> CFGBuilder::buildFor(core::Context ctx, const ast::ClassDef &cd) {
    auto symbol = cd.symbol == core::Symbols::root() ? ctx.state.lookupStaticInitForFile(ctx.file)
                                                     : ctx.state.lookupStaticInitForClass(cd.symbol);
...
unique_ptr<CFG> CFGBuilder::buildFor(CFGContext cctx, unique_ptr<CFG> res, absl::Span<const ast::ExpressionPtr> params,
    ...
    BasicBlock *cont;
    {
        ...
        for (const auto &stat : stats) {
            if (shouldBuildFor(cctx.ctx, stat)) {
                cont = walk(cctx.withTarget(cctx.newTemporary(core::Names::statTemp())), stat, cont);
            }
        }
        cont = walk(cctx.withTarget(retSym), expr, presentCont);
    }
